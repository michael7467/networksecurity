...
def init_dagshub():
    """
    Initialize DagsHub only if explicitly enabled.
    This prevents EC2/Docker startup crashes caused by interactive OAuth.
    """
    try:
        enable_dagshub = os.getenv("ENABLE_DAGSHUB", "false").lower() == "true"
        if not enable_dagshub:
            logging.info("DagsHub is disabled. Skipping dagshub.init().")
            return
        ...
        if not dagshub_token:
            logging.warning("ENABLE_DAGSHUB is true but DAGSHUB_TOKEN is missing. Skipping DagsHub init.")
            return

        os.environ["DAGSHUB_USER_TOKEN"] = dagshub_token

        dagshub.init(
            repo_owner="michael7467",
